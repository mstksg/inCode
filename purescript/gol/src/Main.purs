...
main = do
  ...
  ready doc do
    ...
    drawer <-
      setupDrawer "#golDrawerCont" { height: 8, width: 8 }
        $ \pts -> do
            let
              bumped = Set.fromFoldable (map (bump 6) pts)
            draw2D bumped
            draw3D bumped
            draw4D bumped
            drawFlat bumped
...
makeElemLinks doc elemMap =
  for_ elemMap
    $ \(Tuple e _) -> do
        let
          groupId = Map.lookup e elementGroupMap

          linkString =
            append "Jump to: "
              <<< intercalate " / "
              <<< flip A.mapMaybe linkStrings
              $ \(Tuple e' (Tuple a b)) -> do
  ...
  linkStrings =
    flip map elemMap \(Tuple e ename) ->
...
linkify doc cb = do
  linkElems <-
    ParentNode.querySelectorAll
      ...
      (Document.toParentNode doc)
  flip traverseNodeList_ linkElems \linkElem ->
    void
      $ runMaybeT do
          ...
          Alternative.guard (not (null res))
          liftEffect
            $ onE click (Element.toEventTarget linkElem) \e -> do
                preventDefault e
                cb res
          pure unit
...
stepper ::
  ...
  Map a (NonEmptySet Int)
stepper expand syms cs =
  Map.mapMaybe (NESet.fromFoldable <<< intMapKeys <<< filterIntMap validLiveCount <<< unionsIntMap <<< fromDList)
    $ foldl
        ( \res (Tuple gIx ds) -> case Map.lookup ds prebaked of
            Nothing -> res
            Just (Tuple here there) ->
              flip (Map.unionWith append) res <<< Map.fromFoldable
  ...
  prebake =
    bimap unionsIntMap unionsIntMap
      <<< foldr
          ( \pIx (Tuple here there) ->
              let
                pNeighbs = syms pIx

                here' =
                  singletonIntMap pIx (Additive 1)
                    List.: pNeighbs
                    List.: here

                there' =
                  singletonIntMap pIx (Additive 2)
                    List.: pNeighbs
                    List.: there
              in
                Tuple here' there'
...
genVecRunIxPascal n mx x = go x mx n
  ...
  go q m k =
    let
      Tuple (Tuple j r) k' = chompPascal q m k
    in
      if m == 1 then
        j List.: k' List.: List.nil
      else
        j List.: go r (m - 1) k'
  ...
  go i j tot x0 allSame p r x ls0 = case List.step ls0 of
    List.Nil ->
      let
        ...
        tot' = tot
      in
        Tuple tot' p' <$ Alternative.guard (not (allSame && x == x0))
    List.Cons l ls -> do
      ...
      let
        ...
        p' =
          p
            * ( factorial res
                  `div`
                    (factorial r * factorial xContrib * factorial lContrib)
              )
...
vecRunNeighbsTree n orig = case List.step gens of
  ...
  go (Tuple vrst lastContrib) =
    NodeF lastContrib case vrst of
      Nothing -> List.nil
      Just { i, j, out, x0, allSame, p, q, r, x, ls0 } -> case List.step ls0 of
        List.Nil ->
          ...
          in
            List.singleton (Tuple Nothing contrib)
        List.Cons l ls -> do
          xlContrib <- safeRange 0 (x + l)
          lContrib <- safeRange (max 0 (xlContrib - x)) (min l xlContrib)
          let
            ...
            contrib =
              defer \_ ->
                ...
                , allSame: allSame'
                , leftovers:
                    A.fromFoldable <<< List.take (mx + 1) <<< (_ <> List.repeat 0) <<< List.reverse
                      $ x'
                      List.: l'
                      List.: List.drop 2 (dropAlong out gens)
                , parts:
                    { left: Nullable.notNull lContrib
...
setupDrawer ::
  ...
  (Array Point2 -> Effect Unit) ->
  Effect Drawer
setupDrawer sel size =
  runFn4 _setupDrawer sel size
    $ pointsToBlocks
    <<< Set.fromFoldable

type GolFlatCallback
  = Array (Array (Lazy (Array { x :: Int, y :: Int, pts :: Array Int }))) -> Effect Unit

foreign import _setupGolFlat ::
  Fn3
    String
    Boolean
...
type Gol3DCallback
  = Array (Lazy (Array { x :: Int, y :: Int, zs :: Array Int })) -> Effect Unit

foreign import _setupGol3D ::
  Fn2
    String
...
type Gol4DCallback
  = Array (Lazy (Array { x :: Int, y :: Int, zws :: Array Int })) -> Effect Unit

foreign import _setupGol4D ::
  Fn2
    String
    { height :: Int, width :: Int, maxT :: Int }
...
drawGolSyms3D sel maxZ =
  runFn4 _drawGolSyms sel maxZ
    { dim: 1
...
blockToFourBit c =
  flip map (A.elemIndex c blockTable) \n ->
    FourBit
      { nw: n `mod` 2 == 1
      , ne: (n `div` 2) `mod` 2 == 1
      , sw: (n `div` 4) `mod` 2 == 1
...
fourBitToPoint2 fb =
  List.mapMaybe (\(Tuple p b) -> if b then Just p else Nothing)
...
pointsToBlocks pts =
  intercalate "." $ flip map (List.range 0 3)
    $ \y ->
        flip foldMap (List.range 0 3)
          $ \x ->
