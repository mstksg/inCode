...
appendTopLinks doc = do
  ...
  flip traverseNodeList_ hs \h -> do
    ...
    let
      topLinkNode = Element.toNode topLink
        ...
        sourceInfo <-
          MaybeT
            $ ParentNode.querySelector
                (ParentNode.QuerySelector ".source-info")
                docPN
        header <-
          MaybeT
            $ ParentNode.querySelector
                (ParentNode.QuerySelector ".article > header")
                docPN
        let
          headerET = Element.toEventTarget header
        liftEffect do
  ...
  flip traverseNodeList_ asides \aside -> do
    ...
    flip (withIndex traverseNodeList_) blks \i blk -> do
      when (i == 0) do
        ...
        let
          clickMeNode = Element.toNode clickMe
      ...
      $ do
          ...
          liftEffect
            $ do
                ...
                DOMTokenList.add cList "hide"
                let
                  preET' = Element.toEventTarget <$> Element.fromNode pre
                for_ preET' \preET -> do
      ...
      $ do
          ...
          liftEffect
            $ do
                ...
                let
                  isPrompt =
                    A.mapMaybe (\p -> Pattern p `stripPrefix` tc)
                      [ "λ", "ghci", "$", "irb", ">>>" ]
