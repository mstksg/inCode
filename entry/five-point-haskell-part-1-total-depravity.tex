...
back to a few distinct and helpful ``points'' --- ``doctrines'', if you may
...
Haskell}}, let's set out to establish a five-point unified framework of the
``Typed Functional Programming'' (and Haskell-derived) programming philosophy
aimed to create code that is maintainable, correct, long-lasting, extensible,
and beautiful to write and work with. We'll try to reference real world case
studies with and actual examples when we can, and also attempt to dispel thought
leader sound-bytes that have become all too popular on Twitter --- ``heresies'',
if you may permit the terminology.
...
Atlassian Outage}, in some part, was the result of passing the wrong type of ID.
The operators were intended to pass \emph{App} IDs, but instead passed
\emph{Site} IDs, and the errors cascaded from there. It goes without saying that
if you have a bunch of ``naked'' IDs, then mixing them up is eventually going to
backfire on you.
...
Convenient and effective\ldots as long as you never accidentally use a
\texttt{SiteId} as an \texttt{AppId} or vice versa. And this is a very easy
delusion to take, if you don't believe in total depravity. However\ldots sooner
or later (maybe in a week, maybe in a year, maybe after you onboard that new
team member)\ldots and someone is going to accidentally pass a site id where an
app id is expected.
...
when we want to. In the untyped situation, we could \emph{only} write
polymorphic code, and in the new situation we can \emph{only} write code for one
Id type.
...
Phantom types gives us a \emph{lot} of low-hanging fruits to preventing
inadvertent bad usages.

The
\href{https://www.theregister.com/2017/04/11/database_deletion_downed_digital_ocean_last_week/}{2017
DigitalOcean outage} outage, for example, partially about the wrong environment
...
\emph{deep} inside a function inside a function inside a function called inside
a call to the prod database. I guarantee it.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{Env} \OtherTok{=} \DataTypeTok{Prod} \OperatorTok{|} \DataTypeTok{Test}
...
\texttt{clearTestEnv}, it will have to unite with \texttt{DbConnection\ Test},
...
\tightlist
\item
  \texttt{String.indexOf()}, \texttt{str.find()}, etc. in many languages returns
  a -1 if the substring is not found
\item
  C's \texttt{fgetc()}, \texttt{getchar()}, returns -1 for \texttt{EOF}. And if
...
However, it's not easy to make a ``integer or not found'' type in C or
...
Returning \texttt{Maybe} or Optional forces he caller to handle:
...
erroring operations using Functor and Monad}. In fact, with cheap ADT's, you can
...
indexing lists can send us into a graveyard spiral. Sometimes the answer and
issue will be a bit more subtle. This is our reminder to never let these
implicit assumptions go unnoticed.
...
\NormalTok{  unames }\OtherTok{\textless{}{-}}\NormalTok{ query conn }\StringTok{"SELECT username FROM users where user\_id = ?"}
...
the \href{https://en.wikipedia.org/wiki/Mars_Polar_Lander}{1999 Mars Polar
Lander} crash this way. Its functions took a boolean based on the state of the
legs:
...
\OtherTok{deployThrustrs ::} \DataTypeTok{LegState} \OtherTok{{-}\textgreater{}} \DataTypeTok{IO}\NormalTok{ ()}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  You might forget to always \texttt{hClose} a file handler, and if your files
...
\item
  If another thread throws an asynchronous exception (like a thread cancel), you
  have to make sure the close still happens!
\end{itemize}
...
\OtherTok{doTheThings ::} \DataTypeTok{Map} \DataTypeTok{Username} \DataTypeTok{FilePath} \OtherTok{{-}\textgreater{}} \DataTypeTok{IO}
...
or canceling of cleanup events. It forces us into a last-in first-out sort of
...
\CommentTok{{-}{-} Guarantees that all handlers will eventually close, even if \textasciigrave{}go\textasciigrave{} crashes}
\OtherTok{doTheThings ::} \DataTypeTok{Map} \DataTypeTok{Username} \DataTypeTok{FilePath} \OtherTok{{-}\textgreater{}} \DataTypeTok{IO}\NormalTok{ ()}
...
(In practice, I actually rarely use \texttt{ResourceT} this way --- instead, I
use it often because it's often more convenient at the base of a monad stack
like \texttt{ConduitT}, \texttt{ListT}, etc. than \texttt{ContT} can be.)
...
machinae}. Instead, it means that that all such states are \emph{fundamentally
...
\subsection{The LLM Elephant}\label{the-llm-elephant}
...
If anything, it might be \emph{the} bottleneck. If we can provide LLMs with
properly ``depravity-aware'' typed code --- or somehow encourage them to write
it by giving them the right iterative tooling --- I (maybe naively) believe this
...
Total depravity is all about using types to \emph{prevent errors}. However, most
discussions about the power of types stop here. Oh, how boring types would be if
this was all they gave us!

Types are also a power tool for \emph{program design}, helping you guide the
creation of your abstractions and pushing you to more expressive and robust
designs. They aren't just for preventing bad code, they're for opening your mind
to new avenues of design that were impossible before.
