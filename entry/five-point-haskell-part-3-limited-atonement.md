...
your language, akin to the [function color
...
-   You should care about unmarked IO if you care about global variables. IO is
    the ultimate unrestricted global variable: any `IO` action can freely modify
    the process environment or the file system: it can call `setEnv` or
...
-   You should care about unmarked IO if you care about safe refactoring and
    ...
    this is *not* a valid program optimization if `mkString` could do IO!
...
-   You should care about unmarked IO if you care about laziness. Granted, this
    ...
    was unmarked IO, the order *does* matter. If there wasn't, they *can't*. And
...
-   You should care about unmarked IO if you care about testing. Because it
    states no external dependency, you can test `mkString` without requiring any
    sandboxing, isolation, or extra interleaving interactions with other
    functions.

### IO

All of the above should show you that we get *some* value out of delineating
pure from impure code. But what might that delineation look like?

Based on [function-color
semantics](https://journal.stuffwithstuff.com/2015/02/01/what-color-is-your-function/),
we could create an opt-in system simply where we agree that all functions that
"do IO" are wrapped in some sort of type constructor. A `String` is a pure
string, which does no IO in the process of evaluating or using it. But an
`IO String` is an IO action that produces a string.
