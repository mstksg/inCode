...
\NormalTok{in  assert : eval testVal === 72}
\end{Highlighting}
\end{Shaded}

\subsection{Limits of Generics}\label{limits-of-generics}
...
\texttt{auto} to appease the compiler. The problematic function is
\texttt{accept}, which is template on \texttt{R}. One trick you can do is to
move the template to the top level, and have
\texttt{Expr\textless{}R\textgreater{}} instead of \texttt{Expr}. The \texttt{R}
represents, now, the type that you eventually fold the \texttt{Expr} into.
...
However, remember that we really want \texttt{R} to be polymorphic, because an
\texttt{Expr} \emph{instance} (or value) has to work for multiple values to be
useful: you might want to evaluate an \texttt{Expr} into an \texttt{int}, or
print it into a \texttt{std::string}. So, you have to use the same
\texttt{Expr\textless{}R\textgreater{}} as both an
\texttt{Expr\textless{}int\textgreater{}} or an
\texttt{Expr\textless{}std::string\textgreater{}}. And, in C++, the only way I
could think of this was to use \texttt{auto} as the type:
...
This works only in C++14 and higher. It works because both
\texttt{Expr\textless{}R\textgreater{}},
\texttt{Expr\textless{}int\textgreater{}},
\texttt{Expr\textless{}std::string\textgreater{}} have the same runtime
representation, but there's no actual \texttt{forall\ r.\ Expr\ r} type possible
in C++. I also don't think you can take the same
\texttt{Expr\textless{}R\textgreater{}} as an argument to a function in a nice
way and have that function use it with both an
\texttt{ExprVisitor\textless{}int\textgreater{}} and an
\texttt{ExprVisitor\textless{}std::string\textgreater{}} (a rank-2
type)\ldots so your mileage may vary. Maybe the C++ committee can have ChatGPT
implement virtual template methods for them and we won't have to worry about it.
