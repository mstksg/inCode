...
\href{https://www.cse.chalmers.se/~nad/publications/danielsson-popl2006-tr.pdf}{non-termination}\footnote{This
  is the ``Fast and Loose Reasoning'' condition (Danielsson et al.).} and other
escape hatches\footnote{Excluding \texttt{unsafePerformIO},
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{consumeInt ::} \KeywordTok{forall}\NormalTok{ r}\OperatorTok{.}\NormalTok{ (}\DataTypeTok{Int} \OtherTok{{-}\textgreater{}}\NormalTok{ r) }\OtherTok{{-}\textgreater{}}\NormalTok{ r}
\end{Highlighting}
\end{Shaded}

This might look exotic, but think about what it \emph{must} do. It receives a
function \texttt{Int\ -\textgreater{}\ r} and must produce an \texttt{r}. The
only way to produce an \texttt{r} for \emph{all possible} \texttt{r} is to apply
the given function to some fixed \texttt{Int} chosen ahead of time. This type is
actually isomorphic to \texttt{Int} itself: the only inhabitant is
\texttt{\textbackslash{}f\ -\textgreater{}\ f\ x} for some fixed
\texttt{x}.\footnote{This is actually the essence of Yoneda lemma I think,
  whatever that is.}

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{consumeInt ::} \KeywordTok{forall}\NormalTok{ r}\OperatorTok{.}\NormalTok{ (}\DataTypeTok{Int} \OtherTok{{-}\textgreater{}}\NormalTok{ r) }\OtherTok{{-}\textgreater{}}\NormalTok{ r}
\NormalTok{consumeInt f }\OtherTok{=}\NormalTok{ f }\DecValTok{42}     \CommentTok{{-}{-} must be a fixed \textasciigrave{}Int\textasciigrave{}, cannot dynamically change}

\OtherTok{consumeString ::} \KeywordTok{forall}\NormalTok{ r}\OperatorTok{.}\NormalTok{ (}\DataTypeTok{String} \OtherTok{{-}\textgreater{}}\NormalTok{ r) }\OtherTok{{-}\textgreater{}}\NormalTok{ r}
\NormalTok{consumeString f }\OtherTok{=}\NormalTok{ f }\StringTok{"hello"}     \CommentTok{{-}{-} must be a fixed \textasciigrave{}String\textasciigrave{}, cannot dynamically change}
\end{Highlighting}
\end{Shaded}
...
\item
  If your type is
  \texttt{Foldable\ t\ =\textgreater{}\ t\ a\ -\textgreater{}\ {[}a{]}}, you
  know that results come from the input, but you can still reorder, duplicate,
  or drop elements.
\item
  If your type is
  \texttt{Traversable\ t\ =\textgreater{}\ t\ a\ -\textgreater{}\ {[}a{]}}, you
  get the free theorem \texttt{toList\ .\ fmap\ f\ ==\ map\ f\ .\ toList}: the
  structure is traversed in a canonical order that commutes with mapping. You
  can't ``peek'' at values to decide the traversal order.
\item
  If your type is
...
\item
  ...
  duplicated the same number of times. So, if you give it \texttt{{[}1,2,3{]}},
  you know the result's length has to be of the form \texttt{3\^{}k} and must
  contain \texttt{3\^{}(k-1)} copies of \texttt{1}, \texttt{2}, and \texttt{3},
  in some order.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{notifyUsers ::}\NormalTok{ [}\DataTypeTok{User}\NormalTok{] }\OtherTok{{-}\textgreater{}} \DataTypeTok{Text} \OtherTok{{-}\textgreater{}} \DataTypeTok{IO}\NormalTok{ ()}
\end{Highlighting}
\end{Shaded}
...
certain users, log personal data, vary behavior based on user properties\ldots{}

Compare that to:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{notifyAll ::} \DataTypeTok{Foldable}\NormalTok{ t }\OtherTok{=\textgreater{}}\NormalTok{ t recipient }\OtherTok{{-}\textgreater{}}\NormalTok{ (recipient }\OtherTok{{-}\textgreater{}} \DataTypeTok{IO}\NormalTok{ ()) }\OtherTok{{-}\textgreater{}} \DataTypeTok{IO}\NormalTok{ ()}
\end{Highlighting}
\end{Shaded}

\texttt{notifyAll} is parametric in \texttt{recipient}. It cannot inspect who it
is notifying. It cannot skip recipients based on their properties or treat
admins differently from regular users. All of that ``policy'' logic is forced to
live in the caller. The broadcasting logic itself \emph{must} be uniform: it
calls \texttt{send} indiscriminately on some fixed subset of the elements. It
can pick a subset to send to, but it can't pick what gets included in that
subset based on the properties of the recipient.
