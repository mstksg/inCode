...
is not. And yet, impurity is not a failure, but a boundary that gives purity its
very meaning.
...
is the ultimate unrestricted global variable: any \texttt{IO} action can freely
...
subexpression elimination. Consider populating data with \texttt{mkString}:
...
\emph{not} a valid program optimization if \texttt{mkString} could do IO!
...
\section{Scoped Environments}\label{scoped-environments}

\section{The Simulated Worlds}\label{the-simulated-worlds}

\section{Pure Short Circuiting}\label{pure-short-circuiting}

\section{Reader}\label{reader}
...
\section{Tagless Final}\label{tagless-final}

\section{Extensible Effects}\label{extensible-effects}

\section{Wait, did I just write a Monad
