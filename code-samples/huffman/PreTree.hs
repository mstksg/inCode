...
-- ptTable: Builds a table of inputs and their encodings, by doing a depth
--      first traversal and accumulating the findings in a Writer monad.
ptTable :: Ord a => PreTree a -> Map a Encoding
ptTable pt = execWriter (go pt [])
  where
    go (PTLeaf x) enc       = tell (x ~: reverse enc)
    go (PTNode pt1 pt2) enc = go pt1 (DLeft  : enc) >>
                              go pt2 (DRight : enc)
    k ~: v                  = M.singleton k v
