---
title: "dhall-jit: A Bytecode Compiler and Interpreter for Dhall"
categories: Haskell
tags: haskell, dhall, compilers, interpreters
create-time: 2026/07/17 14:00:00
identifier: dhall-jit
slug: dhall-jit-bytecode-compiler-interpreter
---
...
closure is later `CALL`ed, the interpreter enters `bodyBank` with
`arg <| env` as the new environment.
...
output field. On collision, right wins. The interpreter just iterates the
vector and indexes into the appropriate operand.
...
Enter a bank, set the environment, execute all instructions (they push/pop
from the stack), then pop the final value as the result.
...
The `VProvided NaturalSucc` is handed to user code. When user code calls it
(via `CALL`), `executeProvidedFn` handles it:
...
progressive application protocol -- the same as `RunClosure` on the Haskell side.

## The Native Backend (Compiled C via GCC)

The native backend translates bytecode directly to C source, compiles it with
GCC -O2, and dlopens the resulting `.so`. Each procedure bank becomes a static
C function; inter-bank calls go through a `dispatch` switch.
...
- The compilation overhead is large (~600ms-3s for GCC) but paid once; the `.so`
  is cached on disk keyed by a hash of the bytecode
...
used) but simple and cache-friendly. No linked-list traversal on variable access,
just `env[idx]`. The waste is bounded because Dhall functions tend to be small.
...
compile-time type information pays off -- we can use the Dhall type checker as
a compilation oracle.
...
encoding the full path as a `NESeq Word16` -- e.g. `FromLeft (0 :<|| [2])`
means "output field comes from left operand, field 0, then sub-field 2."

**Builtins as partially-applied values**: `PUSH_BUILTIN` pushes a `VBuiltin`
with an empty arg list. Each `CALL` adds one arg. When the arg count hits
the builtin's arity, it fires. This means builtins don't need special syntax --
