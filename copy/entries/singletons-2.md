...
## A Dependently Typed Outlook

We now have a few good tools to do some real useful dependently typed
programming in Haskell leveraging singletons, and have a way to unite all of
these tools under a common interface.

I did note at the beginning of the first post that dependent types are coming
soon in Haskell.  Let's take a moment to briefly discuss what purpose
*singletons* serves us today in that context.

At the end of the day, we really want to be able to write functions like:

```haskell
lockAnyDoor :: (s :: DoorState) -> Door s -> Door 'Locked
```

Where we can directly observe and pattern match on the type and decide what to
do based on it.  This requires actual dependent types, but for now, with
singletons, we write:

```haskell
lockAnyDoor :: Sing (s :: DoorState) -> Door s -> Door 'Locked
```

To "emulate" it with a runtime witness.

We also want to be able to write functions like:

```haskell
loadDoor :: FilePath -> IO (s ** Door s)
```

Where the `(s ** Door s)` dependent tuple notation signifies an existentially
quantified `s` type that is chosen by `loadDoor` at runtime.

For now, we can use singletons with Haskell's encodings of existentially
quantified types:

```haskell
data SomeDoor = forall s. SomeDoor (Sing s) (Door s)
loadDoor :: FilePath -> IO SomeDoor

withLoadedDoor
    :: FilePath
    -> (forall s. Sing s -> Door s -> IO r) -> IO r
```

I mention this here before moving on to remind you of the "big picture" of
*why* singletons is important in the current state of Haskell.  We really
*want* to be able to pattern match and branch on types, and we really *want* to
be able to encode existential types using the quantified type directly.
However, "until then", we have `Sing s` instead of `s`, and `forall s. SomeDoor
(Sing s) (Door s)`, instead of `s ** Door s`.

You can see, syntactically, the appeal.  The two are sort of designed to look
very syntactically similar.  For now, I hope when you read and write the
Haskell encodings, you can really "see" the true dependently typed versions
that is really being written behind the syntax.
...
The code is available [here][samples] for you to play around with yourself!
