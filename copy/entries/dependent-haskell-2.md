...
part of its usage/API.  You can store them in a list or MVar --- `[OpaqueNet 10
3]` and `MVar (OpaqueNet 10 3)`, serialize/deserialize them without knowing
their internal structure in advance (`loadNet :: FilePath -> IO (OpaqueNet 10
3)`), etc. (if you wanted to load a `Network`, you would need to know exactly
what internal structure was stored, in advance).  Though `Network` is a much
easier type when writing the *implementation*, `OpaqueNet` is a often much more ideal
type to *use*.
...
type (`hs`) *and* the data structure.  (If we had implemented it as `ONet ::
Sing hs -> Network i hs o -> OpaqueNet i o`, this would be slightly clearer!)
...
to always write as much of your program as possible in the "pure" world, and to
...
an existential data type, you have to work with the values in the constructor
...
*   Constructor-style is necessary for writing typeclass instances.  You can't
    ...
    instances later for serialization/deserialization, and it only works with
    constructor-style.
...
*   Haskell doesn't allow you to use Rank-N types as arguments to type
    ...
    o r]` or `[(forall hs. Network i hs o -> r) -> r]`.  You can have `IO
    (OpaqueNet i o)`, but not `IO ((forall hs. Network i hs o -> r) -> r)`.
    These are known as *impredicative* types, which are a big no-no in GHC
    Haskell.  Don't even go there!  The constructor style is necessary in these
    situations unless you want to do a ContT-style skolemization, like `(forall
    hs. Network i hs o -> [r]) -> [r]` and `(forall hs. Network i hs o -> IO r)
    -> IO r`.

*   When writing functions that *take* existentials as inputs, the
    constructor-style is arguably more natural.  For example, we wrote a
    function to find the number of hidden layers in a network earlier:
...
you!  However, for GADTs like `Network`, we have to things manually.
...
type*.  If we want to deserialize a `Network 5 '[10,6,3] 2`, we *know* we want
three `(:&~)`'s and one `O`.
...
them, we have to encode it as a flag in the binary serialization.  We can write
a simple function to get the `[Integer]` of a network's structure:
...
Note that `natVal` and `hiddenStruct` take type-level
information (`n`, `hs`) and turns them into term-level values (`Integer`s,
`[Integer]`s).  They are kind of the opposites of our reification
...
is where things *really* start to get fun.  For the most part, except for what
