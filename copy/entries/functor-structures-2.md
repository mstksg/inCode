...
the functions to "recombine" the `b` and `c` back to `a`. We also note that the
...
directly into `Ap` itself, using [`Pre`][Pre].  This is a trick I first saw in
the *[unjson][]* library. A  value of type:
...
constant...which works out because each of the `Field`s inside could work off
...
embed covariant capabilities in `Dec`.  A value of type:
...
constant...which works out because each of the `Choice`s inside could be
...
Again, we can make a `Dec (Post r Choice) a` using `injectPost`, which asks us to provide
that "embed the `a` in the `r`" function up-front:
