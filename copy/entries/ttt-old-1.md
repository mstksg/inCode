...
```haskell
!!!ttt-old/Part1.hs "data InPlay"1
```
...
```haskell
!!!ttt-old/Part1.hs "data Update"1
```
...
```haskell
!!!ttt-old/Part1.hs "data GameState"
```
...
```haskell
!!!ttt-old/Part1.hs "data SelFound"
```
...
```haskell
!!!ttt-old/Part1.hs "selFoundTest1"
```
...
```haskell
!!!ttt-old/Part1.hs "selFoundTest2"
```
...
```haskell
!!!ttt-old/Part1.hs "selFound" "selFound_znil"2 "selFound_zcons"4 "selFound_snil"3 "selFound_scons"5
```
...
1.  For the first branch, we have `'Z` and `'[]`.  This should be
    ...
    *-Werror=incomplete-patterns* to be sure!  GHC will then reject the program
    ...
    ```haskell
    !!!ttt-old/Part1.hs "selFound_znil"
    ```
...
2.  For the second branch, we have `'Z` and `(x ': xs)`.  We want to
    ...
    ```haskell
    !!!ttt-old/Part1.hs "selFound_zcons"
    ```
...
3.  For the third branch, we have `'S n` and `'[]`.  Again, this should be
    ...
    ```haskell
    !!!ttt-old/Part1.hs "selFound_snil"
    ```
...
4.  The fourth branch is the most interesting one.  We have `'S n` and `(x ':
    ...
    *   If it doesn't, then we can't have an item at `'S n` spot in `x ': xs`
    ...
    ```haskell
    !!!ttt-old/Part1.hs "selFound_scons"
    ```
...
!!!ttt-old/Part1.hs "type OutOfBounds"
```
...
```haskell
!!!ttt-old/Part1.hs "intToN" "getN" "printBoard"
```
